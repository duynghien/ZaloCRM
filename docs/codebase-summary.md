...
│       │   │   ├── copilot/  # Conversational Copilot (Single-Inference Service, Prompt Builder with context pruning, Resilient Parser, Debouncer, Anomaly Escalator, Cache)
│       │   │   ├── quick-replies/ # Thư viện tin nhắn mẫu tổ chức (/ shortcut trigger, chống trùng 409, Socket.IO sync)
│       │   │   └── tags/     # Nhãn hội thoại & liên kết đa tổ chức (quota 6 nhãn/hội thoại, cascade delete, Socket.IO sync)
...
│       │   │   │   ├── ai-feedback-distillation-service.ts # Tiếp nhận góp ý, LLM distillation, auto-activate, safe fallback
│       │   │   │   ├── ai-feedback-routes.ts          # Feedback REST endpoints (Owner/Admin)
│       │   │   │   └── ai-knowledge-prompt-formatter.ts # Đóng gói thẻ <verified_operational_knowledge>, budget < 600 tokens
...
└── frontend/                 # Frontend Single Page Application (Vue 3 + Vuetify 4 + Neo-Brutalism CQA)
    ...
    └── src/
        ...
        │   ├── chat/         # AccountRail, ConversationList, ConversationTagBar, ConversationTagDialog, ConversationTagAssignMenu, MessageThread (Coordinator), MessageThreadHeader, MessageBubbleItem, MessageInputToolbar, SafetyComposeBar, QuickReplySelector, QuickRepliesManagerDialog, MessagePhotoGrid, StagedMediaBar, MediaLightboxDialog (Gallery), ChatCopilotBar, ChatAiDraftCard, ChatAnomalyBanner, ChatAppointments, ChatOrders
...
| **chat & copilot** | Quản lý hội thoại, tin nhắn đa phương tiện 2 chiều, khay chờ tệp đính kèm, dán ảnh clipboard, phóng to ảnh lightbox, lọc tin, thu hồi (undo) an toàn, cập nhật ảnh realtime qua `chat:message:attachments-updated`, hiển thị fallback card khi lỗi ảnh; Thư viện tin nhắn mẫu tổ chức (`QuickReply`) kích hoạt bằng phím tắt `/` an toàn bộ gõ tiếng Việt IME; Nhãn hội thoại đa chiều (`ConversationTag` & `ConversationTagAssignment`) tùy biến bảng màu Neo-Brutalism, giới hạn 6 nhãn/hội thoại, cascade delete sạch sẽ và đồng bộ real-time Socket.IO; kèm Trợ lý Ảo Bán Hàng (Conversational Copilot: Single-Inference AI Engine với context pruning tiết kiệm 25-35% tokens, Smart Turn Debouncer 3.0s, gợi ý phản hồi `Alt+1/2/3`, bóc tách đơn/lịch Human-in-the-Loop, phát hiện bất thường & leo thang quản lý). | `chat-routes.ts`, `message-handler.ts`, `quick-reply-routes.ts`, `quick-reply-service.ts`, `conversation-tag-routes.ts`, `conversation-tag-service.ts`, `conversation-tag-assignment-service.ts`, `chat-copilot-service.ts`, `chat-turn-debouncer.ts`, `chat-copilot-routes.ts`, `chat-copilot-prompt-builder.ts`, `chat-copilot-parser.ts`, `chat-copilot-anomaly-escalator.ts`, `chat-copilot-cache.ts` |
...
| **orders** | Quản lý đơn hàng bán hàng gắn với contact/conversation, cấp mã đơn hàng tuần tự nguyên tử `ORD-YYYYMMDD-NNN`, chi tiết sản phẩm `OrderItem`, phân quyền điều chỉnh giá/chiết khấu, khóa tài chính `order-invoice-lock.ts` khi đã xuất/đối soát hóa đơn KiotViet, làm tròn tiền VNĐ cấp dòng và phân bổ phần dư, serialize an toàn BigInt/Decimal. Kiến trúc modularized (< 200 dòng/file). | `order-routes.ts` (aggregator), `routes/order-crud-routes.ts`, `routes/order-read-handlers.ts`, `routes/order-create-handler.ts`, `routes/order-update-handler.ts`, `routes/order-delete-handler.ts`, `routes/order-stats-routes.ts`, `routes/order-kiotviet-routes.ts`, `routes/order-route-helpers.ts`, `order-code-service.ts`, `order-item-validation.ts`, `order-item-totals.ts`, `order-invoice-lock.ts`, `order-response-serializer.ts` |
...
GET    /api/v1/ai-reports/knowledge-rules     # Danh sách quy tắc tri thức 3 cấp (Owner/Admin)
POST   /api/v1/ai-reports/knowledge-rules     # Thêm mới quy tắc tri thức thủ công (Owner/Admin)
PUT    /api/v1/ai-reports/knowledge-rules/:id # Cập nhật quy tắc tri thức (Owner/Admin)
DELETE /api/v1/ai-reports/knowledge-rules/:id # Xóa quy tắc tri thức (Owner/Admin)
PATCH  /api/v1/ai-reports/knowledge-rules/:id/toggle # Bật/tắt nhanh quy tắc tri thức (Owner/Admin)
GET    /api/v1/ai-reports/knowledge-branches  # Danh sách thẻ chi nhánh đã sử dụng (Owner/Admin)
POST   /api/v1/ai-reports/reports/:id/feedback # Gửi góp ý báo cáo & tự động chắt lọc quy tắc (Owner/Admin)
GET    /api/v1/ai-reports/reports/:id/feedbacks # Lịch sử góp ý của một báo cáo (Owner/Admin)
GET    /api/v1/ai-reports/feedbacks           # Danh sách toàn bộ góp ý trong tổ chức (Owner/Admin)
```
