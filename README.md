...
Hệ thống quản lý tập trung nhiều tài khoản Zalo cá nhân/doanh nghiệp trên 1 giao diện web chuẩn Neo-Brutalism CQA. Chat real-time với thanh chọn tài khoản trực quan (`AccountRail`), tin nhắn đa phương tiện 2 chiều kèm khay chờ (`StagedMediaBar`), dán ảnh clipboard (`Ctrl+V`), phóng to lightbox (`MediaLightboxDialog`), Trợ lý ảo bán hàng Copilot (Single-Inference AI, debouncer 3.0s, Smart Reply `Alt+1/2/3`, bóc tách đơn/lịch Human-in-the-Loop, cảnh báo bất thường), đồng bộ tin nhắn từ thiết bị ngoài (`selfListen`), gắn nhãn chi nhánh & 12 bảng màu nhận diện, đường ống CRM, đặt lịch hẹn, tích hợp KiotViet (đồng bộ danh mục sản phẩm, chi tiết sản phẩm đơn hàng, thanh toán thực thu độc lập, hàng đợi xuất hóa đơn bền vững Outbox Pattern, khóa an toàn tài chính & đối soát Admin), kiểm toán nhóm định kỳ & báo cáo AI đa nhà cung cấp (Gemini, OpenAI, DeepSeek, Local Gateway), theo dõi chi phí & đo lường AI telemetry (`AiCostKpiCard`, AI Usage Report, xuất Excel), phát tin việc cần làm Zalo, REST API & Webhook.
